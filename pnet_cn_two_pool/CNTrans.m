...
% (Partial) defoliation in June % TTR
removal_fraction = 0.75; % TTR
export_factor    = 3;    % TTR
if clim.year (rstep) == 1910 && clim.doy (rstep) == 166 % TTR
  share.FolMass = share.FolMass * (1.0 - removal_fraction); % TTR
  share.FolLitM = share.FolLitM + (share.FolMass * ((1.0 - removal_fraction) / export_factor)); % TTR
%  share.FolLitM = share.FolLitM + (share.FolMass * (1.0 - removal_fraction)); % TTR
end % TTR

RootTurnover = veg.RootTurnoverA - (veg.RootTurnoverB * share.NetNMinLastYr) + (veg.RootTurnoverC * power(share.NetNMinLastYr,2));
if RootTurnover > 2.5
  RootTurnover = 2.5;
...
if clim.year(rstep)>=site.agstart && clim.year(rstep)<site.agstop
  ...
  share.WoodMass = share.WoodMass * (1 - site.agrem * (share.dayspan(rstep) / 365));
  share.WoodMassN = share.WoodMassN * (1 - site.agrem * (share.dayspan(rstep) / 365));
end
