...
if site.POOLS == 0                                                    % TTR
  share.BudC = share.PlantC * 0.4;                                    % TTR
elseif (site.POOLS == 1) || (site.POOLS == 2)                         % TTR
  share.BudC = ((veg.FolMassMax - share.FolMass) * veg.CFracBiomass); % TTR
end                                                                   % TTR
...
% Update NSC pools                                                    % TTR
if site.POOLS == 0                                                    % TTR
  share.WoodC  = share.PlantC * 0.6;                                  % TTR
  share.PlantC = 0.0;                                                 % TTR
...
elseif site.POOLs == 2                                                % TTR
  ...
  % Calculate total NSC carbon left                                   % TTR
  share.PlantC = share.PlantCFast + share.PlantCSlow                  % TTR
end                                                                   % TTR



% Check that wood there is enough wood to support foliage carbon
if share.WoodC < (veg.MinWoodFolRatio * share.BudC)
  TotalC = share.WoodC + share.BudC;
  share.WoodC = TotalC * (veg.MinWoodFolRatio / ...
                          (1.0 + veg.MinWoodFolRatio));
...
if CN_Mode==1
  ...
  % Check that there is enough N available for foliage or adjust foliage C
  % and N accordingly
